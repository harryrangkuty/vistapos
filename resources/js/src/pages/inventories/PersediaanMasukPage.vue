...
<template>
    <a-row type="flex" justify="center">
        <a-col :span="24">
            <a-card class="card" title="Persediaan - Transaksi Masuk">
                <template #extra>
                    <a-row type="flex" style="gap: 1em">
                        <a-col flex="auto">
                            <a-input v-model:value="filter.search" @keyup.enter="readData" placeholder="Ketikkan Kode atau Nomor Invoice"
                                style="width: 300px">
                                <template #addonAfter>
                ...
                <a-table :scroll="{ x: 800 }" :columns="columns" :row-key="(obj) => obj.id" :pagination="_pagination" :loading="loadingStatus"
                    ...
                    <template #bodyCell="{ index, column, record }">
                        ...
                        <template v-if="column.key === 'details_sum_kuantitas'">
                            {{ fb(record.details_sum_kuantitas, 0) }}
                        </template>
    ...
    <a-modal v-model:open="showModal" title="Tambah Data" width="700px" @ok="writeData" :mask-closable="false">
        <a-form ref="form" name="basic" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
            ...
            <a-form-item label="Suplier" data-column="suplier_id" :rules="[{ required: true }]">
                <a-select v-model:value="form.suplier_id" placeholder="--Pilih Suplier--">
                    <a-select-option :key="suplier.id" v-for="suplier in constant.SUPLIER_OPT" :value="suplier.id">
                        {{ suplier.nama }}
                    </a-select-option>
                </a-select>
            </a-form-item>
            ...
            <a-form-item label="Tanggal Invoice/Surat" data-column="tanggal_invoice" :rules="[{ required: true }]">
                <a-date-picker v-model:value="form.tanggal_invoice" :value-format="date_format" :allow-clear="false"
                    placeholder="tanggal invoice/surat" />
            </a-form-item>
