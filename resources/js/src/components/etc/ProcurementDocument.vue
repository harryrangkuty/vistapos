...
export default {
    ...
    methods: {
        ...
        async uploadFile() {
            ...
            const response = await vm.axios.post(vm.writeRoute + '?req=file_upload&code=' + vm.parent.kode, formData).catch((e) => vm.$onAjaxError(e));
            if (response && response.data) {
                vm.inputFile = null;
        ...
        async previewFile(file) {
            ...
            if (response && response.data) {
                ...
                if (window.innerWidth <= 992) {
                    document.getElementById('pdf-modal' + vm.parent.kode).data = url;
                    document.getElementById('pdfx-modal' + vm.parent.kode).src = url;
                } else {
                    document.getElementById('pdf-' + vm.parent.kode).data = url;
                    document.getElementById('pdfx-' + vm.parent.kode).src = url;
                }
    ...
    <a-row type="flex" justify="space-between" class="pb-6">
        <a-col :span="24" :lg="9" class="pr-2.5 !flex flex-col mb-5 lg:mb-0">
            <div class="shadow-md rounded-[8px] overflow-hidden mb-4">
                <div class="flex items-center bg-[#77828c] text-white text-lg px-6 py-2 font-medium">
                    <span>DETAIL DOKUMEN</span>
                    <a-tooltip placement="topRight"
                        :title="!this.form.tgl_surat || !this.form.no_surat ? 'Isi detail dokumen' : null">
                        <Icon v-if="!this.form.tgl_surat || !this.form.no_surat" icon="line-md:alert-circle-twotone"
                            class="text-red-400 ml-1 h-5 w-5" />
                    </a-tooltip>
                </div>
                <a-card class="card file-upload">
                    <div class="w-full flex justify-end">
                        <a-tooltip placement="topLeft" class="w-full sm:w-auto"
                            :title="getTooltipMessage('Mengubah Detail Dokumen')">
                            <a-popconfirm title="Yakin menyimpan detail dokumen?" @confirm="save()"
                                :disabled="!operator_aset || locked">
                                <a-button type="primary" class="w-full sm:w-auto mb-4" :disabled="locked || !operator_aset">
                                    Simpan</a-button>
                            </a-popconfirm>
                        </a-tooltip>
                    </div>
            ...
            <div class="shadow-md rounded-[8px] overflow-hidden mb-4">
                <div class="flex items-center bg-[#77828c] text-white text-lg px-6 py-2 font-medium">
                    ...
                    <a-tooltip placement="topRight" :title="!models.length > 0 ? 'Upload dokumen' : null">
                        <Icon v-if="!models.length > 0" icon="line-md:alert-circle-twotone"
                            class="text-red-400 ml-1 h-5 w-5" />
                    </a-tooltip>
                </div>
                <a-card class="card file-upload">
                    <a-input type="file" data-column="file" accept=".jpg, .jpeg, .png, .pdf" v-model:value="inputFile"
                        ...
                        <template #suffix>
                            <a-tooltip placement="topLeft" class="w-full sm:w-auto"
                                :title="getTooltipMessage('Mengunggah Dokumen')">
                                <a-button @click="uploadFile" class="w-full sm:w-auto" type="primary"
                                    :disabled="locked || !operator_aset">
                                    <template #icon>
                    ...
                    <div class="mt-4 px-2 overflow-y-auto" style="max-height: 60vh;">
                            ...
                            <a-col>
                                <a-button-group style="gap: 5px;">
                                    ...
                                    <a-tooltip v-if="operator_aset" placement="topLeft"
                                        :title="locked ? `Tidak Dapat Menghapus Dokumen Karena Status telah ${this.parent.status === 'FINISH' ? 'Teregistrasi' : 'Terkunci'}` : null">
                                        <a-popconfirm @confirm="deleteFile(file.name)" :disabled="locked"
                                            title="Yakin menghapus file ini?">
        ...
        <a-col :span="24" :lg="15" class="pl-2.5">
            ...
            <object :id="'pdf-' + parent.kode" type="application/pdf" class="rounded-[8px]"
                style="width: 100%; height: 100%">
                <embed :id="'pdfx-' + parent.kode" type="application/pdf" />
        ...
        <a-modal v-model:open="showModal" :title="selectedFileName" width="900px" :footer="null">
            ...
            <object :id="'pdf-modal' + parent.kode" type="application/pdf" class="rounded-[8px] !w-full !h-96">
                <embed :id="'pdfx-modal' + parent.kode" type="application/pdf" />
                <param name="view" value="fit" />
            </object>
        </a-modal>
    </a-row>
