...
class TransactionTypeController extends AuthController
{
    public function __invoke(Request $request)
    {
        $constant = [];

        $title = 'Manajemen Jenis Transaksi';

        $vue = "<transaction-type-page :title='" . json_encode($title) . "' :constant='" . json_encode($constant) . "' />";
    ...
    public function read(Request $request)
    {
        if ($request->req == 'table') {
            $data = TransactionType::where(function ($q) use ($request) {
    ...
    public function write(Request $request)
    {
        ...
        //     $data = TransactionType::find($request->id) ?? new TransactionType();
        ...
        //     $data = TransactionType::find($request->id);
        ...
        //     $data = TransactionType::onlyTrashed()->find($request->id);
