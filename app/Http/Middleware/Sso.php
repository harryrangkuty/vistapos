...
class Sso
{
    ...
    public function createUserFromSso($payload)
    {
        ...
        if ($payload->type == 3) {
            if (!empty($payload->profile?->angkatan)) {
                $user->enrollment_year = $payload->profile->angkatan;
            } else {
                $angkatanPrefix = substr($payload->identifier, 0, 2);
                $angkatan = (int)('20' . $angkatanPrefix);
                $user->enrollment_year = $angkatan;
            }
        } else {
        ...
        // Mapping tipe user pusat data -> role VISTA POS
        $typeToRole = [
            1 => 'lecturer',      // dosen
            2 => 'staff',    // staff akademik (opsional: bisa restricted dulu)
            3 => 'student',       // mahasiswa
            4 => 'restricted',    // alumni (tidak perlu akses aktif)
            5 => 'administrator', // biro / rektorat
        ];

        // Default ke restricted kalau type tidak dikenal
        $roleName = $typeToRole[$payload->type] ?? 'restricted';
